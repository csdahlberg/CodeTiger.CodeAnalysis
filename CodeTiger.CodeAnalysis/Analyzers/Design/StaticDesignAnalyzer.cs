...
public class StaticDesignAnalyzer : DiagnosticAnalyzer
{
    ...
    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics
        => ImmutableArray.Create(ClassesWithAllStaticMembersShouldBeStaticDescriptor);
    ...
    private static void AnalyzeClassForAllStaticMembers(SyntaxNodeAnalysisContext context)
    {
        ...
        {
            // The class either does not have any members (and so should not be made static) or has some non-static
            // members (and so cannot be made static).
            return;
        }
